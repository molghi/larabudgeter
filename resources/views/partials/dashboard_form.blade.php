...
<div class="border border-[var(--accent)] rounded">
  <div class="p-4 bg-black text-[var(--accent)] rounded">
    ...
    <h4 class="mb-3 text-center text-xl font-bold text-[var(--accent3)]">Add Entry</h4>

    <form action="{{ route('entry.create') }}" method="POST" class="space-y-4">
      @csrf

      <div>
        <label for="amount" class="block text-[var(--accent2)] font-bold">Amount <span class="text-red-400">*</span></label>
        <input name="amount" id="amount" type="number" placeholder="Enter amount" class="w-full p-2 border border-[var(--accent2)] rounded bg-black text-[var(--accent)]">
        @error('amount')
      ...
      <div>
        ...
        <input name="note" id="notes" type="text" placeholder="Enter notes" class="w-full p-2 border border-[var(--accent2)] rounded bg-black text-[var(--accent)]">
        @error('note')
            <div class="text-[red] text-sm p-2">{{$message}}</div>
        @enderror
      </div>

      <button class="w-full py-2 border border-[var(--accent)] transition rounded hover:text-black hover:bg-[var(--accent)]">Add</button>
    </form>
